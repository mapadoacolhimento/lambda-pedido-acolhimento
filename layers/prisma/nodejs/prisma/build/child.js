...
class Config {
  ...
  constructor(state, defaultSchema) {
    this.state = state;
  ...
  async all() {
    try {
      const data = await fs__default["default"].readFile(
        this.state.cache_file,
        "utf8"
      );
...
async function getSignature(signatureFile) {
  ...
  signatureFile =
    signatureFile ||
    path__default["default"].join(dirs.cache, PRISMA_SIGNATURE); // new file for signature
...
async function createSignatureFile(signatureFile, signature) {
  ...
  await fs__default["default"].writeFile(
    signatureFile,
    JSON.stringify(signatureState, null, "  ")
  );
...
(function (module) {
  ...
  function parseHost(input, isSpecialArg) {
    ...
    const asciiDomain = tr46$1.toASCII(
      domain,
      false,
      tr46$1.PROCESSING_OPTIONS.NONTRANSITIONAL,
      false
    );
    if (asciiDomain === null) {
      return failure;
...
async function check(state) {
  ...
  urlObj.query = {
    ...
    previous_client_event_id: await _asyncOptionalChain([
      await config.get("output"),
      "optionalAccess",
      async (_) => _.client_event_id,
    ]),
